...
//            {
//                // Devuelve un error si el correo no se proporciona
...
namespace BackendAE.Controllers
{
    ...
    public class UsuariosController : ControllerBase
    {
        ...
        public async Task<ActionResult> CrearUsuario([FromBody] UsuarioCreacionDTO dto)
        {
            ...
            var emailExistente = await _context.Usuarios.FirstOrDefaultAsync(u => u.Email == dto.Email);
            if (emailExistente != null)
            {
                return BadRequest("El correo electrónico ingresado ya está registrado. Por favor, intente con uno diferente.");
            }
        ...
        public async Task<ActionResult> ActualizarUsuario(int id, [FromBody] UsuarioActualizacionDTO dto)
        {
            // 1. Verificar si el usuario a actualizar existe
            var usuario = await _context.Usuarios.FindAsync(id);
            if (usuario == null) return NotFound();

            // 2. ✅ Validar si el nuevo email ya existe en otro usuario
            var emailExistente = await _context.Usuarios
                .FirstOrDefaultAsync(u => u.Email == dto.Email && u.UsuarioId != id);

            if (emailExistente != null)
            {
                return BadRequest("El correo electrónico ingresado ya está registrado para otro usuario. Por favor, intente con uno diferente.");
            }

            // 3. ✅ Validar si el nuevo NombreUsuario ya existe en otro usuario
            // Solo si el DTO tiene un campo para el nombre de usuario
            var nombreUsuarioExistente = await _context.Usuarios
                .FirstOrDefaultAsync(u => u.NombreUsuario == dto.NombreUsuario && u.UsuarioId != id);

            if (nombreUsuarioExistente != null)
            {
                return BadRequest("El nombre de usuario ingresado ya está en uso. Por favor, intente con uno diferente.");
            }

            // Mapea solo los campos actualizables
            _mapper.Map(dto, usuario);

            // Si la actualización incluye el nombre de usuario, actualízalo
            // Nota: esto asume que NombreUsuario está en UsuarioActualizacionDTO
